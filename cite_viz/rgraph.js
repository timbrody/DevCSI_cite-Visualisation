...
function init(){
    ...
    var rgraph = new $jit.RGraph({
        ...
        Events: {
            enable: true,
             onClick: function(node, eventInfo, e) {
                if( node )
                    rgraph.queryData(node);
            },
            onMouseEnter: function(node, eventInfo, e) {
                //viz.canvas.getElement().style.cursor = 'pointer';
                var style = e.target.style;
                rgraph.label.prevColor = style.color;
                style.color = "#FFF";
            },
            onMouseLeave: function(node, eventInfo, e) {
              //viz.canvas.getElement().style.cursor = '';
                var style = e.target.style;
                style.color = rgraph.label.prevColor;
            }
        },
        ...
        onPlaceLabel: function(domElement, node){
            ...
            console.log("label name: "+domElement.innerHTML);
            console.log("label width: "+w);
        }
    });
    rgraph.label = {prevColor: ""};

    rgraph.renderData = function(json){
        //load JSON data
        //rgraph.graph.empty();
    ...
    rgraph.callServer = function(){
        jQuery.ajax({
            url: "http://10.79.254.60:8888/opencitations.net/http%3A%2F%2Fdx.doi.org%2F10.1080%2F10529410390892052?jsonp=foo",
            contentType: "application/json",
            dataType: "jsonp",
            jsonpCallback: 'foo',
            success: function(data, textStatus, jqXHR){
                console.log("ajax success: "+ JSON.stringify(data)+", "+textStatus+", "+JSON.stringify(jqXHR));
                rgraph.renderData(data);
            },
    ...
    rgraph.queryData = function(node){
        console.log("node.id, node.name, node.data :"+node.id+", "+node.name+", "+JSON.stringify(node.data));
        rgraph.callServer();
    };
